...
void ModeAuto::update() {
	...
	case Auto_WP: {
		...
		if (!g2.wp_nav.reached_destination() || g2.wp_nav.is_fast_waypoint()) {
			...
			_distance_to_destination = rover.current_loc.get_distance(
					g2.wp_nav.get_destination());
		}
		break;
	}
	...
	case Auto_HeadingAndSpeed: {
		if (!_reached_heading) {
			...
			calc_throttle(
					calc_speed_nudge(_desired_speed,
							is_negative(_desired_speed)), true);
...
bool ModeAuto::check_trigger(void) {
	...
	if (!is_zero(g.auto_kickstart)) {
		...
		if (xaccel >= g.auto_kickstart) {
			gcs().send_text(MAV_SEVERITY_WARNING, "Triggered AUTO xaccel=%.1f",
...
bool ModeAuto::dock_complt(){

	return rover.mode_dock.dock_complete();
}
...
void ModeAuto::send_guided_position_target() {
	...
	if ((guided_target.last_sent_ms == 0)
		...
		if (GCS_MAVLINK::find_by_mavtype(MAV_TYPE_ONBOARD_CONTROLLER, sysid,
				compid, chan)) {
			gcs().chan(chan - MAVLINK_COMM_0)->send_set_position_target_global_int(
...
bool ModeAuto::start_command(const AP_Mission::Mission_Command &cmd) {
	...
	case MAV_CMD_DO_SET_ROI:
		if (cmd.content.location.alt == 0 && cmd.content.location.lat == 0
...
void ModeAuto::exit_mission() {
	...
	if (g2.mis_done_behave == MIS_DONE_BEHAVE_ACRO
			&& rover.set_mode(rover.mode_acro, ModeReason::MISSION_END)) {
		return;
	}

	if (g2.mis_done_behave == MIS_DONE_BEHAVE_MANUAL
			&& rover.set_mode(rover.mode_manual, ModeReason::MISSION_END)) {
		return;
	}
...
bool ModeAuto::verify_command(const AP_Mission::Mission_Command &cmd) {
	...
	default:
		// error message
		gcs().send_text(MAV_SEVERITY_WARNING, "Skipping invalid cmd #%i",
				cmd.id);
		// return true if we do not recognize the command so that we move on to the next command
		return true;
	}
...
void ModeAuto::do_nav_delay(const AP_Mission::Mission_Command &cmd) {
	...
	gcs().send_text(MAV_SEVERITY_INFO, "Delaying %u sec",
...
void ModeAuto::do_nav_set_yaw_speed(const AP_Mission::Mission_Command &cmd) {
	...
	_desired_speed = constrain_float(cmd.content.set_yaw_speed.speed,
			-speed_max, speed_max);
...
bool ModeAuto::verify_nav_wp(const AP_Mission::Mission_Command &cmd) {
	...
	if (!previously_reached_wp) {
		...
		if (loiter_duration > 0) {
			...
			gcs().send_text(MAV_SEVERITY_INFO, "Reached waypoint #%u",
...
bool ModeAuto::verify_dock_delay(const AP_Mission::Mission_Command &cmd) {
		if (!start_dock_success) {
			if (start_dock()) {
				start_dock_success = true;
				gcs().send_text(MAV_SEVERITY_CRITICAL,
						"Dock Started, Timeout disengaged");
			}
		} else {
//        	gcs().send_text(MAV_SEVERITY_NOTICE, "Dock: failed");
		}
	if(dock_complt()) {
		gcs().send_text(MAV_SEVERITY_NOTICE, "Dock Verified");
		start_dock_success=false;
		return true;
	}
//    gcs().send_text(MAV_SEVERITY_INFO, "Dock remaining time: %u", (unsigned int)loiter_duration);
	if(!start_dock_success){
		if (loiter_duration == 0 ) {
			gcs().send_text(MAV_SEVERITY_INFO, "Dock: Timeout");
			return true;
		} else {
...
void ModeAuto::do_change_speed(const AP_Mission::Mission_Command &cmd) {
	...
	if (set_desired_speed(cmd.content.speed.target_ms)) {
		gcs().send_text(MAV_SEVERITY_INFO, "speed: %.1f m/s",
